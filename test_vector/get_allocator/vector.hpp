...
class vector {
 ...
 public:
  ...
  vector() : _start(0), _end(0), _capa_end(NULL), _alloc {
    std::cout << "Create vector by Defalut vector constructor" << std::endl;
  };
  explicit vector(const Allocator& alloc) {
    std::cout << "Create vector by Copy Allocator vector constructor"
              << std::endl;
  };
  explicit vector(size_type count, const value_type& value = value_type(),
                  const Allocator& alloc = Allocator()) {
    std::cout << "Create vector by specialization constructor" << std::endl;
  };
  ...
  ~vector() { std::cout << "Destructs vector" << std::endl; };

  // get_allocator for c++98 __ returns the associated allocator
  allocator_type get_allocator() const { return _allocate; };
